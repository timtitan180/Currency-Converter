...
var inputAmount = document.QuerySelector("input")[0];
var result = document.getElementbyId("result");
const button = document.getElementbyId("submit-button");
function checkAmount () {
if (inputAmount.value.length < 1 || typeof(inputAmount == NaN) { //checking if the user entered an amount and if that value is an integer
    ...
    FirstDivlist[i].addEventListener("click",function() {
        for(var i = 0; i<FirstDivlist.length; i++) {
              this.value = data.base;//item that was clicked = the base value of the data object
        }
    }
    ...
    SecondDivList[j].addEventListener("click",function() {
      for(var j = 0; j<SecondDivList.lenght; i++) {
            ...
            var conversion = inputAmount * Math.round(this);//calculating the base Amount * the rate thats rounded to a decimal place? still not sure about this one
    }
